...
double** Model::alpha(vector<int>* seq_obs){
    ...
    for (int t = 1; t < T; t++) {
        for (int j = 0; j < n; j++) {
            ...
            for (int i = 0; i < n; i++)
                tmp[i] = res[t-1][i] + A[i][j];
            res[t][j] = ln_of_sum(tmp, 0, n-1) + B[j][(*seq_obs)[t]];
            delete[] tmp;
        }
    }
...
double** Model::beta(vector<int>* seq_obs){
    ...
    // Initialisation : beta[T-1][i] = log(1) = 0
    for (int i = 0; i < n; i++)
        res[T-1][i] = 0.0;
    ...
    for (int t = T-2; t >= 0; t--) {
        for (int i = 0; i < n; i++) {
            double* tmp = new double[n];
            for (int j = 0; j < n; j++)
                tmp[j] = A[i][j] + B[j][(*seq_obs)[t+1]] + res[t+1][j];
            res[t][i] = ln_of_sum(tmp, 0, n-1);
            delete[] tmp;
        }
    }
...
vector<int>* Model::viterbi(vector<int>* seq_obs)
{
    ...
    // delta[t][i] = max log-prob d'être en état i au temps t ayant émis o_1..o_t
    double** delta = new double*[T];
    // psi[t][i] = état prédécesseur optimal menant à l'état i au temps t
    int** psi = new int*[T];
    for (int t = 0; t < T; t++) {
    ...
    // Terminaison : meilleur état final
    double best = -numeric_limits<double>::infinity();
    int best_state = 0;
    for (int i = 0; i < n; i++) {
        if (delta[T-1][i] > best) {
    ...
    // Rétropiste
    vector<int>* res = new vector<int>(T);
    (*res)[T-1] = best_state;
    for (int t = T-2; t >= 0; t--)
        (*res)[t] = psi[t+1][(*res)[t+1]];
...
vector<double>* Model::posterior(vector<int>* seq_obs, vector<int>* seq_state)
{
    ...
    double** alpha_mat = Model::alpha(seq_obs);
    double** beta_mat  = Model::beta(seq_obs);
    double denom = Model::pgen(seq_obs, alpha_mat);

    vector<double>* res = new vector<double>(T);
    for (int t = 0; t < T; t++) {
        int s = (*seq_state)[t];
        (*res)[t] = alpha_mat[t][s] + beta_mat[t][s] - denom;
    }
...
{
    int T = seq_obs->size();
    return ln_of_sum(alpha[T-1], 0, n-1);
}

double*** Model::xi(vector<int>* seq_obs, double* likelihood) 
{
    // assertion: seq_obs is not empty
...
double Model::baum_welch_EMstep(vector<vector<int>*>* data)
{
    ...
    for (int k=0; k<data->size(); k++)
    {
        ...
        for (int i=0; i<n; i++)
        {
            ...
            double* sum = new double[seq_obs->size()-1];
            for (int t=0; t<seq_obs->size()-1; t++)
                sum[t] = gamma[t][i];
            ...
            {	
                for (int t=0; t<seq_obs->size()-1; t++)
                    sum[t] = xi[t][i][j];
                new_A_num[i][j] = ln_of_sum(new_A_num[i][j], ln_of_sum(sum,0,seq_obs->size()-2));
            }
            delete[] sum;
        }
...
{
    /* --------------- Learn pi -------------- */
    int* count_start_state = new int[n];
    for (int i = 0; i < n; i++) count_start_state[i] = 0;

    /* --------------- Learn A -------------- */
    int** count_bigram_state = new int*[n];
    for (int i = 0; i < n; i++) {
        count_bigram_state[i] = new int[n];
        for (int j = 0; j < n; j++) count_bigram_state[i][j] = 0;
    }
    int* count_unigram_state = new int[n];
    for (int i = 0; i < n; i++) count_unigram_state[i] = 0;

    /* --------------- Learn B -------------- */
    int** count_state_obs = new int*[n];
    for (int i = 0; i < n; i++) {
        count_state_obs[i] = new int[M];
        for (int j = 0; j < M; j++) count_state_obs[i][j] = 0;
    }

    /* --------------- Comptage ------------------ */
    int N = data_obs->size();
    for (int k = 0; k < N; k++) {
        vector<int>* obs_seq   = (*data_obs)[k];
        vector<int>* state_seq = (*data_state)[k];
        int T = obs_seq->size();
        ...
        for (int t = 0; t < T; t++) {
            ...
            count_state_obs[s][o]++;
            if (t < T-1) {
                int s_next = (*state_seq)[t+1];
                count_bigram_state[s][s_next]++;
                count_unigram_state[s]++;
            }
        }
    }

    /* --------------- Calcul des probabilités (log) -------------- */
    // pi[i]
    for (int i = 0; i < n; i++) {
        if (count_start_state[i] > 0)
            pi[i] = log((double)count_start_state[i] / N);
        else
            pi[i] = LOG_MINUS_INFTY;
    }

    // A[i][j]
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (count_bigram_state[i][j] > 0)
    ...
    for (int j = 0; j < n; j++) {
        int total = 0;
        for (int k = 0; k < M; k++) total += count_state_obs[j][k];
        for (int k = 0; k < M; k++) {
            if (count_state_obs[j][k] > 0)
                B[j][k] = log((double)count_state_obs[j][k] / total);
            else
                B[j][k] = LOG_MINUS_INFTY;
        }
    }
    ...
    delete[] count_start_state;
    delete[] count_unigram_state;
    for (int i=0; i<n; i++)
        delete[] count_bigram_state[i];
    delete[] count_bigram_state;
    for (int i=0; i<n; i++)
        delete[] count_state_obs[i];
    delete[] count_state_obs;
}
